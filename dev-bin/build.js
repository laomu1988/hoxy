...
co(function*() {
    ...
    let docsDir = path.join(__dirname, '..', 'docs')
        ...
        , fileNames = fs.readdirSync(docsDir).filter(tester).filter(noTemplate)
        , mainPage = fs.readFileSync(pageName, 'utf8')
        , $ = cheerio.load(mainPage)

    for (let fileName of fileNames) {
        let page = fs.readFileSync(path.join(docsDir, fileName), 'utf8')
